...
//Uniforms
this.createUniform("Roughness", "u_roughness", LS.TYPES.NUMBER, 0.3, {min: 0.0, max: 1.0, step: 0.01, precision: 2, widget: "slider"});
...
this.createUniform("Translucency scale", "u_translucency_scale", LS.TYPES.NUMBER, 1000.0, {min: 0.0, max: 10000.0, step: 10, precision: 0});
...
this.createSampler("Transmitance LUT", "u_transmitance_lut_texture", {missing: "black", wrap: gl.CLAMP_TO_EDGE});
this.createSampler("Specular LUT", "u_specular_lut_texture", {missing: "black", wrap: gl.CLAMP_TO_EDGE});
...
void main() {
  ...
  vec3 detailed_normal = perturbNormal(N, V, v_uvs, texture2D(u_normal_texture, v_uvs).rgb);
  ...
  float specular = texture2D(u_specular_texture, v_uvs).x;
  ...
  vec4 lspace_pos = u_light_matrix * vec4(v_pos - u_shadow_shrinking * N, 1.0); //Shrinking explained bt Jimenez et al
  ...
  vec3 transmittance = T(s) * u_light_color * albedo * E * attenuation;
  vec3 reflectance = KS_Skin_Specular(detailed_normal, L, V, u_roughness, specular) * u_light_color * lit;
  
	gl_FragData[0] = vec4(ambient+diffuse, 1.0);
  gl_FragData[1] = vec4(transmittance+reflectance, 1.0);
  gl_FragData[2] = vec4(0.0);
  #ifdef BLOCK_FIRSTPASS
  	gl_FragData[2] = vec4(depth, 0.0, sssMask, 1.0);
  #endif
}
